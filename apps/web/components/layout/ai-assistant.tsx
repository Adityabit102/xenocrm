...
function detectActionIntent(text: string): "create_segment" | "create_campaign" | "schedule_campaign" | "preview_segment" | null {
    const lower = text.toLowerCase();
    const createSegment = ["create segment", "make segment", "build segment", "new segment for", "create a segment"];
    const scheduleCampaign = ["schedule campaign", "schedule a campaign", "schedule whatsapp", "schedule sms", "schedule email", "set campaign for", "campaign at ", "campaign on ", "campaign tomorrow", "campaign next"];
    const createCampaign = ["create campaign", "launch campaign", "send campaign", "run campaign", "new campaign for"];
    const previewSegment = ["show segment", "preview segment", "how many customers", "count customers"];
...
export function AIAssistant() {
    ...
    const renderActionCard = (action: ActionResult) => {
        ...
        if (action.type === "campaign_scheduled") return (
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <CheckCircle2 style={{ width: 13, height: 13, color: "#FFB547", flexShrink: 0 }} />
                    <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.6rem", color: "#FFB547", letterSpacing: "0.1em" }}>
                        CAMPAIGN SCHEDULED
                    </span>
                </div>
                <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", color: "#EDF0FF", margin: 0, marginBottom: 4 }}>
                    <strong>{action.campaignName}</strong>
                </p>
                <p style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.65rem", color: "#FFB547", margin: 0, marginBottom: 8 }}>
                ...
                <button
                    ...
                    style={{ width: "100%", padding: "5px 10px", background: "rgba(255,181,71,0.1)", border: "1px solid rgba(255,181,71,0.25)", borderRadius: 7, color: "#FFB547", fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", fontWeight: 700, cursor: "pointer" }}
                >
                    View in Scheduler →
                </button>
            </div>
        ...
        if (action.type === "segment_created") return (
            <div style={{
                marginTop: 10, padding: "10px 14px",
                background: "rgba(0,226,147,0.07)",
                border: "1px solid rgba(0,226,147,0.2)",
                borderRadius: 10,
            }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <CheckCircle2 style={{ width: 13, height: 13, color: "#00e293", flexShrink: 0 }} />
                    <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.6rem", color: "#00e293", letterSpacing: "0.1em" }}>
                        SEGMENT CREATED
                    </span>
                </div>
                <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", color: "#EDF0FF", margin: 0, marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                    <button
                        ...
                        style={{ flex: 1, padding: "5px 10px", background: "rgba(0,226,147,0.12)", border: "1px solid rgba(0,226,147,0.25)", borderRadius: 7, color: "#00e293", fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", fontWeight: 700, cursor: "pointer" }}
                    >
                        View Segment →
                    </button>
                    <button
                        onClick={() => { router.push("/campaigns/new"); setOpen(false); }}
                        style={{ flex: 1, padding: "5px 10px", background: "rgba(91,95,239,0.12)", border: "1px solid rgba(91,95,239,0.25)", borderRadius: 7, color: "#c0c1ff", fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", fontWeight: 700, cursor: "pointer" }}
                    >
                        Create Campaign
                    </button>
                </div>
        ...
        if (action.type === "campaign_created") return (
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <CheckCircle2 style={{ width: 13, height: 13, color: "#c0c1ff", flexShrink: 0 }} />
                    <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.6rem", color: "#c0c1ff", letterSpacing: "0.1em" }}>
                        CAMPAIGN CREATED
                    </span>
                </div>
                <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", color: "#EDF0FF", margin: 0, marginBottom: 8 }}>
                ...
                <button
                    ...
                    style={{ width: "100%", padding: "5px 10px", background: "rgba(91,95,239,0.12)", border: "1px solid rgba(91,95,239,0.25)", borderRadius: 7, color: "#c0c1ff", fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", fontWeight: 700, cursor: "pointer" }}
                >
                    View Campaign →
                </button>
            </div>
        ...
        if (action.type === "segment_preview") return (
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.6rem", color: "#7B82A0", letterSpacing: "0.1em" }}>
                        SEGMENT PREVIEW
                    </span>
                </div>
                <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.75rem", color: "#EDF0FF", margin: 0, marginBottom: 4 }}>
                    {action.insight}
                </p>
                <p style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.65rem", color: "#00e293", margin: 0, marginBottom: 10 }}>
                    {action.customerCount?.toLocaleString()} matching customers
                </p>
                <button
                    onClick={() => handleSegmentAction(action.query, true)}
                    style={{ width: "100%", padding: "5px 10px", background: "rgba(0,226,147,0.1)", border: "1px solid rgba(0,226,147,0.25)", borderRadius: 7, color: "#00e293", fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", fontWeight: 700, cursor: "pointer" }}
                >
                    Save this segment →
                </button>
            </div>
        ...
        if (action.type === "navigated") return (
            <button
                ...
                style={{ marginTop: 8, width: "100%", padding: "5px 12px", background: "rgba(91,95,239,0.1)", border: "1px solid rgba(91,95,239,0.2)", borderRadius: 8, color: "#c0c1ff", fontFamily: "DM Sans,sans-serif", fontSize: "0.72rem", fontWeight: 700, cursor: "pointer" }}
            >
                → Go to {action.label}
            </button>
        );
        ...
        if (action.type === "error") return (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8, padding: "6px 10px", background: "rgba(255,76,106,0.07)", border: "1px solid rgba(255,76,106,0.2)", borderRadius: 8 }}>
                <AlertCircle style={{ width: 12, height: 12, color: "#FF4D6A", flexShrink: 0 }} />
                <span style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", color: "#FF4D6A" }}>{action.message}</span>
            </div>
        );
    ...
    return (
        <>
            ...
            <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 100, display: "flex", alignItems: "center", gap: 10 }}>
                {showLabel && (
                    <div style={{
                        ...
                        fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", fontWeight: 600, color: "#EDF0FF",
                        whiteSpace: "nowrap",
                        ...
                        <button
                            ...
                            style={{ background: "none", border: "none", cursor: "pointer", color: "#7B82A0", padding: 0, display: "flex", alignItems: "center", transition: "color 0.15s", lineHeight: 1 }}
                ...
                <button
                    ...
                    style={{
                        ...
                        background: "#0D1017", border: "1.5px solid #252D48",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        cursor: "pointer", boxShadow: "0 4px 20px rgba(0,0,0,0.5)",
                        transition: "all 0.2s", padding: 8, overflow: "hidden", flexShrink: 0,
                    }}
                    onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 24px rgba(91,95,239,0.4)")}
                    onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.boxShadow = "0 4px 20px rgba(0,0,0,0.5)")}
                >
                    <Image src="/xeno-logo.png" alt="Xeno AI" width={28} height={28} style={{ objectFit: "contain", filter: "invert(1)" }} />
                </button>
            </div>
            ...
            {open && (
                <>
                    <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 98, background: "rgba(0,0,0,0.4)" }} />
                    ...
                    <div style={{
                        ...
                        boxShadow: "0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(192,193,255,0.05)",
                        overflow: "hidden",
                    }}>

                        {/* Header */}
                        <div style={{ padding: "14px 16px", borderBottom: "1px solid #1A2035", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(0,0,0,0.2)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#000", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", border: "1px solid #252D48" }}>
                                    <Image src="/xeno-logo.png" alt="X" width={20} height={20} style={{ objectFit: "contain" }} />
                                </div>
                                <div>
                                    <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: "0.85rem", color: "#EDF0FF" }}>Xeno Intelligence</div>
                                    <div style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.62rem", color: "#00e293", display: "flex", alignItems: "center", gap: 4 }}>
                                        <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#00e293", display: "inline-block", animation: "xai-pulse 2s infinite" }} />
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                {messages.length > 0 && (
                                    <button
                                        onClick={() => setMessages([])}
                                        title="Clear chat"
                                        style={{ background: "none", border: "none", color: "#464555", cursor: "pointer", padding: 4, fontSize: "0.65rem", fontFamily: "JetBrains Mono,monospace", transition: "color 0.15s" }}
                                        onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.color = "#7B82A0")}
                                        onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.color = "#464555")}
                                    >
                                        CLEAR
                                    </button>
                                )}
                        ...
                        <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 8px", display: "flex", flexDirection: "column", gap: 10 }}>
                            {messages.length === 0 && (
                                <div style={{ textAlign: "center", padding: "16px 0" }}>
                                    <div style={{ fontFamily: "Syne,sans-serif", fontSize: "0.9rem", fontWeight: 700, color: "#EDF0FF", marginBottom: 6 }}>
                                        Hi! I&apos;m Xeno AI 👋
                                    </div>
                                    <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.75rem", color: "#7B82A0", lineHeight: 1.5, marginBottom: 14, padding: "0 4px" }}>
                                        I can <strong style={{ color: "#EDF0FF" }}>create segments</strong>, <strong style={{ color: "#EDF0FF" }}>launch campaigns</strong>, <strong style={{ color: "#EDF0FF" }}>schedule campaigns</strong>, navigate pages, or answer questions.
                                    </p>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
                                        {[
                                            ...
                                            <button
                                                key={prompt}
                                                onClick={() => send(prompt)}
                                                style={{ padding: "7px 12px", background: "rgba(91,95,239,0.07)", border: "1px solid rgba(91,95,239,0.15)", borderRadius: 8, color: "#c0c1ff", fontFamily: "DM Sans,sans-serif", fontSize: "0.72rem", fontWeight: 500, cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
                                                onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = "rgba(91,95,239,0.14)")}
                                                onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = "rgba(91,95,239,0.07)")}
                                            >
                                                {prompt}
                                            </button>
                                        ))}
                                    ...
                                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center" }}>
                                        {QUICK_ACTIONS.map(a => (
                                            <button
                                                ...
                                                style={{ padding: "4px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid #1A2035", borderRadius: 99, color: "#7B82A0", fontFamily: "DM Sans,sans-serif", fontSize: "0.68rem", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4, transition: "all 0.15s" }}
                                                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = "#EDF0FF"; (e.currentTarget as HTMLButtonElement).style.borderColor = "#252D48"; }}
                                                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = "#7B82A0"; (e.currentTarget as HTMLButtonElement).style.borderColor = "#1A2035"; }}
                                            >
                            ...
                            {messages.map((msg, i) => (
                                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start" }}>
                                    <div style={{
                                        ...
                                        background: msg.role === "user" ? "#5b5fef" : "rgba(255,255,255,0.04)",
                                        border: msg.role === "user" ? "none" : "1px solid #1A2035",
                                        fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", color: "#EDF0FF", lineHeight: 1.55,
                                        whiteSpace: "pre-wrap",
                                    }}>
                                        {msg.text}
                                    </div>
                            ...
                            {loading && (
                                <div style={{ display: "flex", justifyContent: "flex-start" }}>
                                    <div style={{ padding: "8px 14px", borderRadius: "12px 12px 12px 2px", background: "rgba(255,255,255,0.04)", border: "1px solid #1A2035", display: "flex", gap: 4 }}>
                                        {[0, 1, 2].map(i => (
                                            <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#c0c1ff", animation: `xai-bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />
                                        ))}
                                    </div>
                                </div>
                            )}
                        ...
                        {/* Voice indicator */}
                        {listening && (
                            <div style={{ padding: "6px 16px", background: "rgba(0,226,147,0.06)", borderTop: "1px solid rgba(0,226,147,0.15)", display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00e293", display: "inline-block", animation: "xai-pulse 0.8s ease infinite" }} />
                                <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: "0.62rem", color: "#00e293", letterSpacing: "0.05em" }}>LISTENING…</span>
                                {input && <span style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.7rem", color: "#7B82A0", marginLeft: 4, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{input}</span>}
                            </div>
                        )}

                        {/* Input */}
                        <div style={{ padding: "10px 12px", borderTop: "1px solid #1A2035", display: "flex", gap: 8, alignItems: "center" }}>
                            <input
                                type="text"
                                value={input}
                                ...
                                style={{ flex: 1, background: "rgba(24,29,46,0.6)", border: "1px solid #252D48", borderRadius: 8, padding: "8px 12px", fontFamily: "DM Sans,sans-serif", fontSize: "0.78rem", color: "#EDF0FF", outline: "none", transition: "border-color 0.15s" }}
                            ...
                            {voiceSupported && (
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: listening ? "rgba(0,226,147,0.15)" : "rgba(255,255,255,0.04)",
                                        display: "flex", alignItems: "center", justifyContent: "center",
                                        cursor: "pointer", transition: "all 0.15s",
                                        boxShadow: listening ? "0 0 12px rgba(0,226,147,0.3)" : "none",
                                    }}
                                    onMouseEnter={e => { if (!listening) (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,255,255,0.08)"; }}
                                    onMouseLeave={e => { if (!listening) (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,255,255,0.04)"; }}
                                >
                                    {listening
                                        ? <MicOff style={{ width: 14, height: 14, color: "#00e293" }} />
                                        : <Mic style={{ width: 14, height: 14, color: "#7B82A0" }} />
                                    }
                                </button>
                            )}
                            ...
                            <button
                                ...
                                style={{ width: 34, height: 34, borderRadius: 8, background: loading || !input.trim() ? "rgba(255,255,255,0.04)" : "#5b5fef", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: loading || !input.trim() ? "not-allowed" : "pointer", transition: "all 0.15s", flexShrink: 0 }}
                            >
                                {loading
                                    ? <Loader2 style={{ width: 14, height: 14, color: "#7B82A0", animation: "xai-spin 1s linear infinite" }} />
