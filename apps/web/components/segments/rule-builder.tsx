...
const inputStyle: React.CSSProperties = {
  background: "rgba(24,29,46,0.6)", border: "1px solid #252D48", borderRadius: 7,
  padding: "7px 10px", fontFamily: "DM Sans,sans-serif", fontSize: "0.82rem",
  color: "#EDF0FF", outline: "none", transition: "border-color 0.15s", boxSizing: "border-box" as const,
};
...
export function RuleBuilder({ value, onChange }: RuleBuilderProps) {
      ...
      return (
        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
          <input type="number" placeholder="Min" value={minVal}
            ...
            onFocus={e => (e.target.style.borderColor = "#5b5fef")} onBlur={e => (e.target.style.borderColor = "#252D48")} />
          <span style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.72rem", color: "#7B82A0" }}>and</span>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, fontFamily: "DM Sans,sans-serif" }}>
      ...
      {}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 12, borderBottom: "1px solid #1A2035" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.82rem", fontWeight: 600, color: "#EDF0FF" }}>Match conditions:</span>
          <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", border: "1px solid #1A2035", borderRadius: 8, padding: 3, gap: 3 }}>
            {(["AND", "OR"] as const).map(l => (
              <button key={l} type="button" onClick={() => updateLogic(l)}
                style={{ padding: "4px 12px", borderRadius: 6, border: logic === l ? "1px solid #252D48" : "1px solid transparent", background: logic === l ? "#13151F" : "transparent", color: logic === l ? "#c0c1ff" : "#7B82A0", fontFamily: "DM Sans,sans-serif", fontSize: "0.72rem", fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>
      ...
      {/* Conditions */}
      {conditions.length === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "28px 16px", border: "2px dashed #252D48", borderRadius: 10, textAlign: "center", background: "rgba(255,255,255,0.02)", gap: 8 }}>
          <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.82rem", color: "#7B82A0", margin: 0 }}>No active filter rules.</p>
          <p style={{ fontFamily: "DM Sans,sans-serif", fontSize: "0.72rem", color: "#464555", margin: 0 }}>Add a condition to filter your customer audience segment.</p>
          <button type="button" onClick={addCondition}
            style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 8, padding: "7px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid #252D48", borderRadius: 8, color: "#EDF0FF", fontFamily: "DM Sans,sans-serif", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}
            onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.borderColor = "#5b5fef")}
            onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.borderColor = "#252D48")}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {conditions.map((cond, idx) => {
            ...
            return (
              <div key={idx} style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, padding: "10px 12px", background: "rgba(255,255,255,0.02)", border: "1px solid #1A2035", borderRadius: 10 }}>
                {}
                ...
                <button type="button" onClick={() => removeCondition(idx)}
                  style={{ width: 32, height: 32, borderRadius: 6, background: "none", border: "none", color: "#464555", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.15s", flexShrink: 0 }}
          ...
          {}
          <button type="button" onClick={addCondition}
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0", background: "transparent", border: "1px dashed #252D48", borderRadius: 8, color: "#7B82A0", fontFamily: "DM Sans,sans-serif", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", marginTop: 4, transition: "all 0.15s", width: "100%" }}
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = "#5b5fef"; (e.currentTarget as HTMLButtonElement).style.color = "#c0c1ff"; }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = "#252D48"; (e.currentTarget as HTMLButtonElement).style.color = "#7B82A0"; }}>
            <Plus style={{ width: 13, height: 13 }} /> + Add Condition
          </button>
        </div>
      )}
